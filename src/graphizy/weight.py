...
def describe_weight_options() -> None:
    """Print available weight computation strategies."""
    print("Enhanced Weight Computation Options:")
    print("\n1. WeightComputer Methods:")
    print("   - 'distance': Compute actual distance values")
    print("   - 'age': Temporal age weights (direct/inverse/exponential)")
    print("   - 'formula': Custom formula evaluation using edge attributes")
    print("   - 'function': Custom Python function")
    print("   - 'combined': Multiple weighted factors")

    print("\n2. Convenience Constructors:")
    print("   - create_distance_computer()")
    print("   - create_weight_from_distance_computer()")
    print("   - create_custom_attribute_computer()")
    print("   - create_age_weight_computer()")
    print("   - create_combined_weight_computer()")

    print("\n3. Real-Time Functions:")
    print("   - compute_distance_and_weight_fast()")
    print("   - setup_realtime_weight_computer()")
    print("   - FastAttributeComputer.compute_multiple_attributes_fast()")

    print("\n4. Advanced Strategies:")
    print("   - threshold_weight_computer()")
    print("   - linear_combination_weight_computer()")

    print("\n5. Configuration Options:")
    print("   - target_attribute: specify any attribute name")
    print("   - auto_add_distance: automatic distance computation")
    print("   - normalize: normalize results to [0,1]")
    print("   - distance_metric: euclidean, manhattan, chebyshev, etc.")
    ...
    print("   weight_computer = create_weight_from_distance_computer('1/(distance + 0.1)')")
    print("   graph = weight_computer.compute_weights(graph, data_points)")
    print("   # Result: graph.es['weight'] contains 1/(distance + 0.1)")

    print("\n3. Compute custom attribute:")
    print("   custom_computer = create_custom_attribute_computer('importance', 'distance * 2')")
    print("   graph = custom_computer.compute_weights(graph, data_points)")
    print("   # Result: graph.es['importance'] contains distance * 2")

    print("\n4. Chain multiple computations:")
    print("   computer = WeightComputer()")
    print("   graph = computer.compute_attribute(graph, data, 'distance', method='distance')")
    print("   graph = computer.compute_attribute(graph, data, 'weight', method='formula', formula='1/distance')")
    print("   graph = computer.compute_attribute(graph, data, 'strength', method='formula', formula='weight * distance')")


def example_realtime_usage():
    """
    Demonstrate real-time optimized weight computation.

    Shows the most efficient patterns for real-time applications.
    """
    print("=== Real-Time Weight Computation Examples ===")

    print("\n1. Ultra-fast distance + weight computation:")
    print("   graph = compute_distance_and_weight_fast(graph, data_points)")
    print("   # Result: graph has both 'distance' and 'weight' attributes")

    print("\n2. Multiple attributes in one pass:")
    print("   fast_computer = FastAttributeComputer()")
    print("   config = {")
    print("       'distance': {'method': 'distance'},")
    print("       'weight': {'method': 'formula', 'formula': '1/(distance + 0.1)'},")
    print("       'strength': {'method': 'formula', 'formula': 'weight * distance'}")
    print("   }")
    print("   graph = fast_computer.compute_multiple_attributes_fast(graph, data, config)")

    print("\n3. Pre-configured for real-time loop:")
    print("   # Setup once")
    print("   fast_comp = setup_realtime_weight_computer(")
    print("       weight_formula='exp(-distance/50)',")
    print("       additional_attributes={")
    print("           'importance': {'method': 'formula', 'formula': 'weight * 10'}")
    print("       }")
    print("   )")
    print("   # Use repeatedly in loop - very fast!")
    print("   for new_data in data_stream:")
    print("       graph = make_graph(new_data)")
    print("       graph = fast_comp.compute_multiple_attributes_fast(graph, new_data, fast_comp._default_config)")


def example_advanced_usage():
    """
    Demonstrate advanced weight computation strategies.
    """
    print("=== Advanced Weight Computation Examples ===")

    print("\n1. Threshold-based weights:")
    print("   threshold_comp = threshold_weight_computer(threshold=50.0, high=1.0, low=0.1)")
    print("   graph = threshold_comp.compute_weights(graph, data_points)")
    print("   # Result: weight = 1.0 if distance <= 50, else 0.1")

    print("\n2. Age-based weights (requires memory system):")
    print("   age_comp = create_age_weight_computer(age_mode='exponential', decay_rate=0.1)")
    print("   graph = age_comp.compute_weights(graph, data_points)")
    print("   # Result: weight = exp(-0.1 * age)")

    print("\n3. Combined factors:")
    print("   combined_comp = create_combined_weight_computer(")
    print("       factors=['distance', 'age'],")
    print("       weights={'distance': 0.7, 'age': 0.3}")
    print("   )")
    print("   graph = combined_comp.compute_weights(graph, data_points)")
    print("   # Result: weight = 0.7 * normalized_distance + 0.3 * normalized_age")

    print("\n4. Custom function:")
    print("   def custom_weight(graph, decay_rate=0.05):")
    print("       distances = graph.es['distance']")
    print("       return [np.exp(-decay_rate * d) for d in distances]")
    print("   ")
    print("   custom_comp = WeightComputer(method='function', custom_function=custom_weight)")
    print("   graph = custom_comp.compute_weights(graph, data_points)")


if __name__ == "__main__":
    describe_weight_options()
    print("\n" + "="*60)
    example_basic_usage()
    print("\n" + "="*60)
